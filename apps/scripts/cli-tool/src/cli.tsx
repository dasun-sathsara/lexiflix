	...
	return (
		<Box flexDirection="column">
			...
			>
				...
				<Box marginTop={1}>
					<Text>
						<Text bold>Total Unique Words:</Text> {totalWords} |{" "}
						<Text bold>Total Occurrences:</Text> {totalOccurrences}
					</Text>
				</Box>
				<Box marginTop={1}>
			...
			{visibleResults.map((word, idx) => {
				...
				const barLength = Math.round((word.count / maxCount) * 20);
			...
			<Box marginTop={1} borderStyle="round" paddingX={2}>
				...
				<Text color="magenta"> │ Press Q or ESC to exit</Text>
			</Box>
		</Box>
	);
};
...
const App: React.FC = () => {
	...
	const handleFileSelect = async (fileName: string) => {
		...
		const pythonPath = join(process.cwd(), '..', 'python-analyzer', '.venv', 'Scripts', 'python.exe');
	...
	return (
		<Box flexDirection="column" padding={1}>
			...
			{state === "displaying" && (
				<ResultsDisplay results={results} fileName={selectedFile} />
			)}
