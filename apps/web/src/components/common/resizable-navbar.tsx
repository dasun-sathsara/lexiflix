  ...
  return (
    <motion.div
      animate={{
        ...
        boxShadow: visible ? "var(--shadow-popover)" : "none",
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "hidden flex-1 items-center justify-center space-x-2 text-sm font-medium text-zinc-600 transition duration-200 hover:text-zinc-800 lg:flex lg:space-x-2",
        className,
      )}
    >
      {items.map((item, idx) => (
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-white px-4 py-8 shadow-popover dark:bg-neutral-950",
            className,
          )}
        >
          {children}
...
export const NavbarButton = ({
  ...
  const variantStyles = {
    primary: "shadow-popover text-black hover:text-black",
    secondary: "bg-transparent shadow-none dark:text-white",
    dark: "bg-black text-white shadow-popover",
    gradient:
