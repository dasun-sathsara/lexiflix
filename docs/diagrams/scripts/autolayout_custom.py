...
NODE_STYLE = "rounded=1;whiteSpace=wrap;html=1;fillColor=#ffffff;strokeColor=#b8651e;strokeWidth=2;fontColor=#1e293b;fontFamily=Comic Sans MS,cursive;fontSize=12;fontStyle=0;sketch=1;shadow=0;"
EDGE_STYLE = "edgeStyle=orthogonalEdgeStyle;rounded=1;curved=1;orthogonalLoop=1;jettySize=auto;html=1;strokeWidth=2;strokeColor=#555555;jumpStyle=arc;jumpSize=6;fontFamily=Comic Sans MS,cursive;fontStyle=0;"
...
def build_dot(graph):
    rankdir = str(graph.get("direction", "TB")).upper()
    if rankdir not in {"TB", "BT", "LR", "RL"}: rankdir = "TB"
    ...
    ranksep = graph.get("ranksep", 2.0)
    nodesep = graph.get("nodesep", 1.0)
    lines = [f"digraph G {{ rankdir={rankdir}; splines=ortho; ranksep={ranksep}; nodesep={nodesep}; node [shape=box fixedsize=true];"]
...
# Hand-drawn organic palette colors for containers
PALETTE_COLORS = {
    "Client": ("#ffe4b5", "#b8651e"),  # Peach / warm border
    "Vercel / Web App": ("#ffe4b5", "#b8651e"),
    "Trigger.dev Cloud": ("#fff4cc", "#b8901a"),  # Light yellow / gold border
    "Azure Container Apps (ACA)": ("#ffd9b3", "#c25100"),  # Light orange / dark orange border
    "Durable Data Stores": ("#def0dc", "#5c8a49"),  # Light green / green border
    "External APIs & Cloud Services": ("#f5e6d3", "#8b7355"),  # Cream / brown border
    "FastAPI Endpoint": ("#ffd9b3", "#c25100"),  # Light orange
    "Linguistic Processing Pipeline": ("#def0dc", "#5c8a49"),
    "Subtitle NLP Pipeline": ("#def0dc", "#5c8a49"),  # Light green
    "Filtering & Scoring Engine": ("#fff4cc", "#b8901a"),  # Light yellow
}

def group_style(p):
    top_group = p[0]
    is_sub_group = len(p) > 1
    
    fill, stroke = PALETTE_COLORS.get(top_group, ("#f5e6d3", "#8b7355"))
    
    font_spec = "fontFamily=Comic Sans MS,cursive;fontStyle=0;"
    
    if is_sub_group:
        # Subgroup has dashed wobbly border
        return (f"rounded=1;whiteSpace=wrap;html=1;fillColor=none;strokeColor={stroke};"
                f"fontColor={stroke};verticalAlign=top;{font_spec}dashed=1;strokeWidth=1.5;sketch=1;arcSize=8;")
    else:
        # Top-level group has rounded corners, solid background, solid wobbly border
        return (f"rounded=1;whiteSpace=wrap;html=1;fillColor={fill};strokeColor={stroke};"
                f"fontColor={stroke};verticalAlign=top;{font_spec}dashed=0;strokeWidth=2;sketch=1;arcSize=8;")
...
def to_drawio(graph, height, pos, edge_pts, color=True):
    ...
    for p in ordered:
        ...
        gstyle = group_style(p) if color else GROUP_STYLE
        cells.append(
            f'        <mxCell id="{attr(gid[p])}" value="{attr(glabel[p].replace('&#xa;', '<br>').replace('\\n', '<br>').replace('\n', '<br>'))}" '
    ...
    for node in nodes:
        ...
        if node.get("style"):
            ...
            style = NODE_STYLE
        # If node has an icon, adjust its style to align text left and pad it
        node_style = style
        if "icon" in node:
            node_style = node_style.rstrip(';') + ";align=left;spacingLeft=58;"
            
        raw_label = node.get("label", nid)
        clean_label = raw_label.replace('&#xa;', '<br>').replace('\\n', '<br>').replace('\n', '<br>')
        label_val = attr(clean_label)
        cells.append(
            f'        <mxCell id="{attr(nid)}" value="{label_val}" '
            f'style="{attr(node_style)}" vertex="1" parent="{attr(parent)}">\n'
            f'          <mxGeometry x="{x}" y="{y}" width="{w}" height="{h}" as="geometry"/>\n'
            f"        </mxCell>"
        )
        
        # Add icon child cell if present
        if "icon" in node:
            icon_url = node["icon"]
            icon_id = f"{nid}_icon"
            iy = snap((h - 32) / 2)
            cells.append(
                f'        <mxCell id="{attr(icon_id)}" value="" '
                f'style="shape=image;html=1;imageAspect=0;aspect=fixed;image={attr(icon_url)};pointerEvents=0;" '
                f'vertex="1" parent="{attr(nid)}">\n'
                f'          <mxGeometry x="15" y="{iy}" width="32" height="32" as="geometry"/>\n'
                f"        </mxCell>"
            )
