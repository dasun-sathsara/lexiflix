...
As outlined in Chapter~\ref{ch:overview}, the system has two asynchronous jobs: reusable content analysis and user-specific pack generation. Both follow the same pattern: a server action records a durable row in Postgres, then enqueues a Trigger task by its typed id.
...
The full SRS algorithm---state machine, interval formulas, ease factor calculations, and
mastery gate---is documented in the companion SRS Algorithm document. This section covers
only the architectural integration point: derived due-state computed from a stored
\texttt{dueAt} timestamp.
